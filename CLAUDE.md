...
CloudflareにデプロイするMCPサーバー本体（メインアプリケーション）
詳細は [`apps/server/CLAUDE.md`](./apps/server/CLAUDE.md) を参照すること。

### 共通パッケージ（`packages/*`）
複数のapp/packageで利用する可能性があるものは、packages/[package name]に独立して用意する。
